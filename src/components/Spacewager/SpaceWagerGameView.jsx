...
export default function SpaceWagerCardHeader(props) {
    ...
    const [paginationLastElementRound, setPaginationLastElementRound] =
        useState(null)
    const [isLoadingMore, setIsLoadingMore] = useState(false)
    const [loaderPendingToResolve, setLoaderPendingToResolve] = useState({
        resolving: false,
        id: null,
    })
    ...
    function gameData() {
        let render = games.map((game) => (
            <tr key={game.game_id}>
                ...
                <td>
                    {game.up != '0'
                        ? numeral(game.up / 1000000).format('0,0.00') + 'UST'
                        : '-'}
                </td>
                <td>
                    {game.down != '0'
                        ? numeral(game.down / 1000000).format('0,0.00') + 'UST'
                        : '-'}
                </td>
                <td>
                    {game.prize != '0'
                        ? numeral(game.prize / 1000000).format('0,0.00') + 'UST'
                        : '-'}
                </td>
                ...
                <td className="text-end">
                        ...
                        game.game_id + 2 < state.spaceWagerCurrentRound ? (
                            <>
                                <button
                                    ...
                                    hidden={game.resolved}
                                    disabled={
                                        loaderPendingToResolve.id ==
                                            game.game_id || isResolvingAll
                                    }
                                    onClick={() => collectPrize(game.game_id)}
                                >
                                    {isResolvingAll ||
                                    loaderPendingToResolve.id ==
                                        game.game_id ? (
                            ...
                            <>
                                ...
                                <span>
                                    {game.game_id >
                                    state.spaceWagerCurrentRound - 3 ? (
                                        <>
                                            open in{' '}
                                            {game.game_id +
                                                3 -
                    ...
                    ) : parseInt(game.up) + parseInt(game.down) ==
                      parseInt(game.prize) ? (
                        <>
                            <Swap size={23} /> Refund
                        </>
            ...
            <div className="w-100 py-4">
                <div className="row">
                    ...
                    <div className="col-4">
                        <button
                            className="btn btn-outline-primary w-100 btn-sm"
                            disabled={!isSomeResolvable || isResolvingAll}
                            onClick={() => resolve_multiples()}
                        >
                            {isResolvingAll ? (
                                <>
                                    Resolving...{' '}
                ...
                <div className="table-responsive">
                    <table className="table text-white">
                        ...
                        <tbody>{gameData()}</tbody>
                    </table>
                </div>
                <button
                    className="btn btn-plain"
                    disabled={!isActivePagination || isLoadingMore}
                    onClick={() => gameUser(paginationLastElementRound)}
                >
                    {isLoadingMore ? (
                        <>
                            Loading more...
