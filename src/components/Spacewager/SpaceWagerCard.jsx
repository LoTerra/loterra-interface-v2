...
export default function SpaceWagerCard(props) {
    ...
    const getVariation = async (lockedPrice, currentPrice) => {
        try {
            ...
            let split_variation = numeral(variation)
                .format('0,0.000000')
                .split('.')
            let format_variation =
                split_variation.length == 1 ? (
                    <>
                        {split_variation[0]}
                        <span style={{ fontSize: '0.8em' }}>.000000</span>
                    </>
                ) : (
                    <>
                        {split_variation[0]}
            ...
            if (variation > 0) {
                setFormattedVariation(
                    <>
                        <TrendUp size={23} /> ${format_variation}
                    </>,
                )
                ...
                setFormattedVariation(
                    <>
                        <TrendDown size={23} /> ${format_variation}
                    </>,
                )
    ...
    const getPersonalBids = async (round) => {
        try {
            let personal_bid_info = await api
                .contractQuery(state.spaceWagerAddress, {
                    game: {
    ...
    return (
        <div className="col-10 col-md-9 mx-auto">
            <div
                className={
                    'card spacewager-card h-100 ' +
                    (isPastPrediction && obj.success != null ? ' finished' : '')
                }
                style={{
            ...
            >
                ...
                <div className="card-body">
                    <div
                        className="btn-green fw-bold w-100"
                        style={{
                            ...
                            <span className="small fw-normal d-block">
                                <span className="fw-bold">
                                    {isNextPrediction
                                        ...
                                        ? '0.00'
                                        : numeral(
                                              (parseInt(bettingOddsOnDown) +
                                                  parseInt(bettingOddsOnUp)) /
                                                  parseInt(bettingOddsOnUp),
                                          ).format('0,0.00')}
                                    x
                                </span>{' '}
                    ...
                    <div
                        ...
                        style={
                            variationStatus == 'DOWN' && !isNextPrediction
                                ? downStyle
                                : variationStatus == 'UP' && !isNextPrediction
                                ? upStyle
                                : { border: '' }
                        }
                    >
                    ...
                    <div
                        className="btn-red w-100 fw-bold"
                        style={{
                ...
                <div className="card-footer p-0">
                    {personalBidInfo &&
                        (parseInt(personalBidInfo.up) > 0 ||
                            parseInt(personalBidInfo.down) > 0) && (
                            <div className="row">
                                ...
                                <div className="col-6 mt-2">
                                    <strong style={{ color: '#17b96b' }}>
                                        UP
                                    </strong>
                                    <p>
                                        {numeral(
                                            personalBidInfo.up / 1000000,
                                        ).format('0,0')}{' '}
                                        UST
                                    </p>
                                </div>
