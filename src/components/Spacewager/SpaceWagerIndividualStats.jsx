...
export default function SpaceWagerIndividualStats(props) {
    ...
    async function getSpacewagerLeaderboardIndividual() {
        try {
            let spacewager_individual_data = await api
                ...
                <div className="card lota-card">
                    <div className="card-body p-2 p-md-3">
                        ...
                        {state.wallet && state.wallet.walletAddress ? (
                            ...
                            <p className="mb-0 fs-6 fw-normal">
                                Connect wallet
                            </p>
                        )}
                    </div>
        ...
        <div className="container-fluid my-5">
            ...
            {individualStats && (
                <div className="row">
                    {stats_div(
                        'Games won',
                        individualStats.game_won,
                        <Trophy size={50} style={{ lineHeight: 0 }} />,
                    )}
                    {stats_div(
                        'Games lost',
                        individualStats.game_over,
                        <Trophy size={50} style={{ lineHeight: 0 }} />,
                    )}
                    {stats_div(
                        'Game rewards',
                        <>
                            {numeral(
                                parseFloat(
                                    individualStats.game_rewards / 1000000,
                                ),
                            ).format('0,0.00')}
                            <span style={{ fontSize: '14px' }}>UST</span>
                        </>,
                        <Trophy size={50} style={{ lineHeight: 0 }} />,
                    )}
                </div>
